...
public class CommandListener {
     ...
     */
    private final String wrongCommand = "  Не верная команда. Для получения справки используйте \"help\".";
    ...
    public Command handle(String[] command) {
        ...
        switch (m.command) {
            case "delete_script": {
                if (m.args.length == 1) {
                    return new DeleteScript(m.args[0]);
                } else {
            ...
            case "delete_file": {
                if (m.args.length == 1) {
                    return new DeleteFile(m.args[0]);
                } else {
                    output.println(wrongArguments);
                    return null;
                }
            ...
            case "create_script": {
                if (m.args.length == 1) {
                    ...
                    return script == null ? null : new CreateScript(m.args[0], script);
            ...
            case "add": {
                ...
                if (m.args.length > 0) {
                    ...
                    return new Add(new HumanBeing[]{newHumanBeing});
                ...
                if (m.args.length > 1) {
                    try {
                        ...
                        if (m.args[1].equalsIgnoreCase("RANDOM")) {
                            if (m.args.length == 2) {
                                newHumanBeing = HumanBeing.getHuman();
                                return new Update(id, newHumanBeing);
                            } else {
                                output.println(wrongArguments);
                                return null;
                            }
            ...
            case "execute_script": {
                if (m.args.length == 1) {
                    return new ExecuteScript(m.args[0]);
                } else {
                    output.println(wrongArguments);
                }
                return null;
            ...
            case "change_file": {
                if (m.args.length == 1) {
                    return new ChangeFile(m.args[0]);
                } else {
                    output.println(wrongArguments);
                }
                return null;
            ...
            case "remove_greater": {
                if (m.args.length > 0) {
                    ...
                    return new RemoveGreater(humanBeing);
            ...
            case "count_by_soundtrack_name": {
                if (m.args.length > 0) {
                    ...
                    return new CountBySoundtrackName(soundtrack);
            ...
            default: {
                try {
                    Command cmd = (Command) (m.constructor).newInstance();
    ...
    /**
     * Возвращает сложенное по честям сообщение, содержащее команду и ее аргументы.
     *
    ...
    private Message commandBuilder(String[] parts) {
        ...
        for (int i = 0; i < parts.length; ++i) {
            ...
            Constructor commandConstructor = commands.get(cmd.toString());
